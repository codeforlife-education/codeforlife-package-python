...
def has_completed_auth_setup(user):
    return (not using_two_factor(user)) or (
        user.userprofile.is_verified and using_two_factor(user)
    )
...
def logged_in_as_school_student(u):
    return (
        logged_in_as_student(u) and not u.userprofile.student.is_independent()
    )
...
def check_teacher_authorised(request, class_teacher):
    ...
    is_current_teacher_school_admin = (
        class_teacher.school == request.user.new_teacher.school
        and request.user.new_teacher.is_admin
    )
