...
from ..models import DailyActivity, Student, Teacher
...
class TestModels(TestCase):
    ...
    def test_sanitise_uk_postcode(self):
        ...
        assert (
            sanitise_uk_postcode(postcode_with_space) == "AL10 9NE"
        )  # Check it stays the same
        assert (
            sanitise_uk_postcode(postcode_without_space) == "AL10 9UL"
        )  # Check a space is added
        assert (
            sanitise_uk_postcode(invalid_postcode) == "123"
