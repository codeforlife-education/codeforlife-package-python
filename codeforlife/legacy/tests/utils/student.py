...
def create_student_with_direct_login(access_code) -> Tuple[Student, str]:
    ...
    student = Student.objects.schoolFactory(
        klass, name, password, hashed_login_id
    )
...
def generate_independent_student_details():
    name = (
        "Independent Student %d" % generate_independent_student_details.next_id
    )
    email_address = (
        "student%d@codeforlife.com"
        % generate_independent_student_details.next_id
    )
...
def create_independent_student(page, mock_send_dotdigital_email):
    ...
    page = email.follow_verify_email_link_to_login(
        page, verification_url, "independent"
    )
