...
def generate_details(**kwargs):
    ...
    email_address = kwargs.get(
        "email_address", f"testteacher{random_int}@codeforlife.com"
    )
    password = kwargs.get(
        "password", "$RFVBGT%6yhn$RFVBGT%6yhn$RFVBGT%6yhn$RFVBGT%6yhn"
    )
...
def signup_duplicate_teacher_fail(
    page, duplicate_email, mock_send_dotdigital_email
):
    page = page.go_to_signup_page()

    first_name, last_name, email_address, password = generate_details()
    page = page.signup(
        first_name, last_name, duplicate_email, password, password
    )

    page = page.return_to_home_page()

    login_link = mock_send_dotdigital_email.call_args.kwargs[
        "personalization_values"
    ]["LOGIN_URL"]

    page = email.follow_duplicate_account_link_to_login(
        page, login_link, "teacher"
    )
...
def verify_email(page, verification_url):
    page = email.follow_verify_email_link_to_login(
        page, verification_url, "teacher"
    )
